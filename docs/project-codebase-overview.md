...
- AI 不只是单路生成，而是通过 PRD Briefing、Spec Battle、Review 等对抗机制**主动把风险暴露给使用者**——用户不需要自己就能看出坑，有个 AI 专门负责把坑指给他看，他只需要裁决。语义上**红方是生产者**、**蓝方是监督者**、**裁决者**依据双方产出做判定，三者都是 Agent；**人类不是红方也不是蓝方**，他在门禁那一层，批准权只在他手里。
