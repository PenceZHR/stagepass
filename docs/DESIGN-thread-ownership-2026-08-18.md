...
## 十、真机实验（2026-08-18 晚，走生产代码那条路）

两条连接接同一个 daemon（`startManagedAppServer` + `AppServerSessionHost`，
不自己拼协议）。**带对照组** —— 不先证「占着的时候连不上」，「放手之后连上了」
就什么都不说明。

### 10.1 第一次：对照组把实验判无效了

```
对照组 · 甲占着时乙 attach → 拒了：no rollout found for thread id 01a017aa…
实验组 · 甲放手后乙 attach → 拒了：no rollout found for thread id 01a017aa…
```

两臂**同一个理由**被拒 —— 那不是 attach 冲突，是**零轮次线程没有 rollout**
（未知第 3 条，这次实测确认）。这次实验测的不是要测的东西，作废。

**这正是对照组存在的理由。** 没有它，「放手之后还是连不上」会被读成「放手没用」。

### 10.2 第二次：先跑一个真 turn，再测

```
线程 01a017ab-35d0-7b42-9b1d-5dd732c1f0d2，跑完一轮之后：
对照组 · 甲占着时乙 attach → 连上了
实验组 · 甲放手后乙 attach → 连上了
```

**协议层没有独占锁。** 两条连接可以同时 attach 同一条线程。

### 10.3 于是三条未知变成了这样

| 原来的未知 | 现在 |
|---|---|
| 1 · 开完线程能不能放手让 App attach | **不再拦路** —— 协议层压根不拦。`"open in another app"` 是 Codex App 自己在 UI 层挡的；StagePass 放手只会让情况变好，不可能变坏。**剩下的那半只有你能验**：StagePass 放手之后 App 让不让你进 |
| 2 · 没人 attach 时要审批的 turn 会怎样 | 未验，但走钩子之后它不再拦路 |
| 3 · 零轮次线程 | **已答，而且是一条硬约束** —— 见下 |

### 10.4 新拿到的硬约束：先跑一轮，线程才存在得起来

一条 `thread/start` 出来、还没跑过 turn 的线程**没有 rollout，谁都 resume 不了**。

所以座位的次序不能是「开线程 → 放手 → 等人来看」。必须是：

```
开线程 → 立刻发第一轮 → 跑完（rollout 落地）→ 放手 → 人才点得开
```

**在第一轮跑完之前，那条线程对人是不存在的。** 界面在这段时间里要说得出这件事，
不能显示一个点开就报错的入口 —— 那正是「回一个正确的状态码不等于把话说清楚」
那条教训的同一种形状。

### 10.5 实验留下的一次性线程

`01a017aa-dd7c-7a91-ab62-79bef72edbbb`（零轮次，没有 rollout）
