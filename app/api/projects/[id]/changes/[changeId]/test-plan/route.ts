...
function testPlanActionId(value: unknown): "run_test_plan" | "retry_test_plan" {
  if (value === undefined) return "run_test_plan";
  if (value === "run_test_plan" || value === "retry_test_plan") return value;
  throw new PreflightValidationError(
    "invalid_preflight_input",
    "actionId must be run_test_plan or retry_test_plan",
  );
}
...
export async function POST(
  ...
  try {
    ...
    const actionId = testPlanActionId(payload.actionId);
