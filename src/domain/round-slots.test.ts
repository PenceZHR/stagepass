...
describe("Round slot file", () => {
  it("hands the model a fully shaped document it only has to fill", () => {
    ...
    assert.deepEqual(doc.slots[0], {
      id: "B-01", severity: null, title: null, where: null, why: null, owner: null,
    });
    assert.deepEqual(doc.slots.at(-1).id, "B-15");
  });
  ...
  it("reads only the filled slots and keeps their order", () => {
    ...
    assert.deepEqual(result.filled, [
      { id: "B-01", severity: "P1", title: "第一条", where: "docs/PRD.md §3", why: "没有可观测的判据", owner: null },
      { id: "B-05", severity: "P0", title: "第五条", where: "docs/PRD.md §3", why: "没有可观测的判据", owner: null },
    ]);
  });
  ...
  it("refuses a half-written slot instead of guessing the rest", () => {
    ...
    const result = readSlotDocument(text, HEAD);
    assert.equal(result.ok, false);
    assert.match(result.reason, /B-03/);
  ...
  it("lays out a question sheet whose options the model never writes", () => {
    ...
    assert.equal(result.ok, true);
    assert.deepEqual(result.filled, [
      { id: "B-01", question: "结算失败时分数保留吗？", why: "PRD 3.2 没写" },
    ]);
  });
  ...
  it("numbers slots so lexical order matches numeric order", () => {
    ...
    assert.equal(ids[0], "B-01");
    assert.equal(ids.at(-1), "B-15");
  });
});
