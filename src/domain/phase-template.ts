...
const REVIEW_SECTIONS: readonly TemplateSection[] = [
  {
    key: "subject",
    title: "审的是哪一个 commit",
    asks: "写出 sha。不要笼统地说「当前代码」—— 这份报告会被下一轮和下一个阶段当事实"
      + "引用。**sha 至少写到前 12 位**（七位以下会撞），再写出这个 commit 的标题；"
      + "审的范围只覆盖它的一部分时，写清覆盖的是哪几个文件。",
  },
  {
    key: "against",
    title: "逐条对照上游的结果",
    asks: "对着 Spec 和 Arch 一条条看的结果，不是通读一遍谈感受。每条写成三样："
  ...
  },
  {
    key: "findings",
    title: "发现的问题",
    asks: "每条指明文件和位置，看的人能直接翻到那儿。没有就明写没有。**位置要到文件"
      + "路径加函数名或行号**，还要写出这条在什么输入、什么状态下会真的出问题 ——"
      + "「错误处理不够完善」既不是位置，也不是条件。",
  },
  {
    key: "severity",
    title: "哪些必须改",
    asks: "区分「必须改」和「可以这样也可以那样」。**按上一节那些 id 逐条归类**，"
      + "不是分成两段笼统地说；判「必须改」的每条写出不改会发生什么（哪条路径会错、"
      + "谁会受影响）—— 没有后果的「必须改」判不了轻重。",
  },
  {
    key: "paths",
    title: "错误路径和边界",
    asks: "查过的错误路径和边界情况，不只是主流程。**逐条写出查的是哪个文件哪个函数"
      + "的哪条分支**（输入为空、越界、下游报错、中途失败），以及查的结果是什么 ——"
      + "「检查了边界情况」没说查了哪一条，和没查看起来一模一样。",
  },
];
...
const MERGE_SECTIONS: readonly TemplateSection[] = [
  {
    key: "what",
    title: "这次改了什么",
    asks: "对谁有影响。不要复述阶段名。**改的东西点到名**：新增、改名、删掉的是哪个"
      + "命令、哪个导出、哪个配置项、哪张表；受影响的写清是哪一类人在哪个动作上会看到"
      + "不一样 ——「优化了流程」既没点名，也没说谁会看到。",
  },
  {
    key: "risks",
    title: "被接受的风险",
    asks: "每一条写理由和影响范围。没有就明写没有 —— 默默放行不算回答了这一节。"
      + "**影响范围点到名**：哪几个功能、哪些文件、哪类数据；理由要写出它在什么条件下"
      + "会真的发作 ——「有一定风险」既没有范围也没有条件，接受的是什么无从说起。",
  },
  {
    key: "rollback",
    title: "怎么回滚",
    asks: "具体怎么做，不只是说「可以回滚」。写出**退的是哪几个 commit**（sha 或"
      + "范围）、跑哪条命令、已经改过的数据要不要跟着回、退完之后看什么确认干净了 ——"
      + "少了最后一样，回滚做没做成没人知道。",
  },
  {
    key: "coverage",
    title: "说明和实际改动对得上",
    asks: "这一次的每个 commit 都在上面的说明里，没有漏掉的。**逐个列出来**：sha 加"
      + "它的标题，后面写清它对应上面说明里的哪一条 ——「所有改动均已说明」是一句自我"
      + "担保，核对不了。",
  },
];
...
const BUILD_SECTIONS: readonly TemplateSection[] = [
  {
    key: "did",
    title: "这一轮改了什么",
    asks: "按 Plan 的哪几步做的，实际动了哪些文件。不要复述 Plan。**文件逐个列出"
      + "路径**，每个后面写清在里面加了改了哪个函数、哪个导出、哪一段；再写出它对应 "
      + "Plan 的第几步（写编号就够了，那一步的内容不用抄过来）——「重构了配置模块」"
      + "没说出一个文件。",
  },
  {
    key: "tests",
    ...
    asks: "跑的是改动本身（编译、启动、走一遍改的路径），命令和输出贴出来。"
      ...
      + "哪一条，留给 TestPlan。命令原样贴（连同在哪个目录下跑的），输出贴出**判它"
      + "成了的那几行原文**，不是一句「编译通过」。",
  },
  {
    key: "deviation",
    ...
    asks: "多做的、少做的、换了做法的，逐条写为什么。一条都没有就明写「没有」。每条要"
      + "指到 Plan 的第几步和实际动的哪个文件，并把两边都写出来：计划说的是什么、实际"
      + "做的是什么 ——「做了些调整」既没指到步，也没说改成了什么。",
  },
  {
    key: "decisions",
    title: "不明显的决定",
...
const RETRO_SECTIONS: readonly TemplateSection[] = [
  {
    key: "happened",
    title: "这一次真实发生了什么",
    asks: "真实发生的事，不是应该发生的事。每条写出**在哪个阶段、第几轮**发生的，"
      + "能数的写数（跑了几轮、开了几条问题、卡了多久）——「过程比较顺利」既没有时点"
      + "也没有数，它对不上库里的任何一条记录。",
  },
  {
    key: "process",
    title: "关于流程本身的",
    asks: "至少一条是关于流程的，不能全是关于代码的。**指到流程里的哪一处**：哪个阶段"
      + "的哪一节、哪条标准、哪道闸门、哪一次打回 ——「流程还需完善」没有指到任何一处，"
      + "下一次也不知道该动什么。",
  },
  {
    key: "next",
    title: "下次会不一样的做法",
    asks: "每条结论落到一个具体的做法。「以后要注意」不是一个做法。做法要写出**动的是"
      + "哪一样东西**（哪份模板的哪一节、哪条标准、哪个阶段的哪一步）、什么时候做、"
      + "以及做完之后凭什么看出它生效了 —— 三样缺一样，下一次就没人做得了它。",
  },
  {
    key: "kept",
    title: "这次做对的事",
    asks: "不只写做错的。每条指到具体的那一次：哪个阶段、哪一轮、做了什么动作，"
      + "以及它当时挡下了什么、省了什么 ——「配合良好」没有指到任何一次，下次也复制不了。",
  },
];
