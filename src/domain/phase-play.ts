...
/**
 * 一个阶段里，红蓝裁三方各自的那一节 —— **十二份（可派发的阶段各一），各写各的。**
 *
 * ## 为什么不是一份模板加几个分支
 *
...
export const PHASE_PLAY: Readonly<Record<Exclude<Phase, "Done">, PhasePlay>> = {
  ...
  Build: {
    ...
    blue: {
      ...
      /*
       * ## 2026-08-06 拍的 Build 分工（PLAN §3.2）：**蓝方跑 TestPlan 交的测试**
       *
       * 红方看不到测试源码、也不许碰它 —— 蓝方在红方写完之后照 TestPlan「怎么跑」
       * 那一节执行，把哪条失败、输出是什么写进判定依据。这不是「看不见会退化」：
       * 红方拿得到「哪条失败、输出是什么」，那是对着一个不可见的判据做 TDD，
       * 不是猜（这一条推翻了 BACKLOG §8.8 的反对理由）。
       *
       * 「放开一样能力要看有没有标准在禁它」：原来这一格写着「不要自己执行任何
       * 东西」，正是在禁这件事 —— 所以改的是这一行本身，而不是在别处开口子。
       */
      reach: "   可以读这一轮改动涉及的文件、它们的直接调用方，以及任务里列出的那几份已批准上游产物（要判它有没有做到要求，就得对着要求看）—— 只读这些，不要把整个仓库读一遍。**要跑 TestPlan 交的测试**：照它「怎么跑」那一节的命令执行一遍，哪条失败、实际输出是什么，写进对应标准的判定依据里。不要动手修代码，也不要改任何测试。",
      investigates: false,
      idRule: [],
      after: [
        "   它不要另外列问题清单：这个阶段的判断全部走逐条判定。代码里真正的缺陷留给 Review —— 那儿的红方就是审代码的。",
        "   再要它在同一个 json 块里多给一个 `overall` 字段：一句话说这一轮整体够不够格、为什么。这一句不挡任何东西，是写给人看的。",
      ],
    },
  },
