-- The delegated Spec round's judge turn.
--
-- The three-turn form dispatched `spec_writer`, `spec_critic` and
-- `spec_verdict` as separate server calls. The delegated form runs one turn
-- whose agent judges and whose red and blue are sub-agents it spawns, so it
-- needs a role of its own: without it the very first delegated round dies on a
-- CHECK violation at insert, before any model is ever called.
--
-- The three old roles stay valid. Rounds already recorded under them keep
-- reading, and the three-turn path is still the one that runs when the
-- delegated form is off.
--
-- SQLite cannot widen a CHECK in place, so the table is rebuilt. The order is
-- the one 0029 established and it matters: renaming the live table first would
-- rewrite every foreign key pointing at it to the temporary name. Nothing
-- currently references `codex_logical_turns`, but building the replacement
-- under its own name and renaming it into place last is the procedure that
-- stays correct when something does.

PRAGMA foreign_keys=OFF;
--> statement-breakpoint
-- Restartable: a run that failed after creating the replacement must not leave
-- it behind to be filled twice.
DROP TABLE IF EXISTS `__0030_codex_logical_turns_new`;
--> statement-breakpoint
CREATE TABLE `__0030_codex_logical_turns_new` (
  ...
  `phase` TEXT NOT NULL,
  `role` TEXT NOT NULL CHECK (`role` IN ('stage','spec_writer','spec_critic','spec_verdict','spec_judge','build','fix','prd_turn','context_select','context_generate','interaction_present','interaction_wakeup')),
...
--> statement-breakpoint
INSERT INTO `__0030_codex_logical_turns_new` (
...
--> statement-breakpoint
ALTER TABLE `__0030_codex_logical_turns_new` RENAME TO `codex_logical_turns`;
--> statement-breakpoint
